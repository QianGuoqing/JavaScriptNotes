var utils = (function () {
    ...
    function addClass(curEle, className) {
        var ary = className.replace(/(^\s+|\s+$)/g, '').split(/\s+/g);
    ...
    function css(curElem) {
        ...
        if (typeof argTwo === 'string') { // -> 第二个参数值是一个字符串，这样的话很有可能在获取样式；为什么说是很有可能，因为还需要判断是否存在第三个参数，如果第三个参数存在的话，不是获取了，而是在单独的设置样式属性值
            var argThree = arguments[2];
            if (!argThree) { // -> 第三个参数不存在
                //return this.getCss(curElem, argTwo);
