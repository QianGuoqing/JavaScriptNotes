 ...
 */
// -> 使用惰性思想（JavaScript高阶编程技巧之一）来封装常用的方法库
// 第一次在给utils赋值的时候，就已经把兼容处理好了，把最后的结果放在flag变量中，以后再每一个方法中
    // 只要是IE6-8不兼容的，就不需要重新的检测，只需要使用flag的值即可
var utils = (function () {
    var flag = 'getComputedStyle' in window; // flag->这个变量不销毁，存储的是判断当前的浏览器是否兼容getComputedStyle,兼容的话是标准浏览器，如果flag=false，说明不兼容，说明当前浏览器是IE6-8
    return {
